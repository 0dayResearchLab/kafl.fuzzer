...
#define DEVICE_NAME         L"\\DosDevices\\mwfsmfltr"
...
void set_ip_range() {
   ...
   { 
        ...
        hprintf("[+] Loaded Module List\n");
        for(int i = 0; i < cDrivers; i++){
            hprintf("%s 0x%p 0x%p\n", (const char *)ModuleInfo->Modules[i].FullPathName, (UINT64)drivers[i], (UINT64)drivers[i] + ModuleInfo->Modules[i].ImageSize);
        }
        ...
        //     if(strstr((const char*)ModuleInfo->Modules[i].FullPathName, DRIVERNAME) > 0 ) {
        //         uint64_t buffer[3];
        //         buffer[0] = (UINT64)drivers[i]; //pt_ip_filter_a
        //         buffer[1] = (UINT64)drivers[i] + ModuleInfo->Modules[i].ImageSize; //pt_ip_filter_b
        //         buffer[2] = 0; //idx
...
int main(int argc, char** argv)
{
    ...
    kafl_vuln_handle = CreateFile((LPCSTR)"\\\\.\\mwfsmfltr", //fasoo
    ...
    hprintf("[+] Attempting to open vulnerable device file1 (%s)\n", "\\\\.\\mwfsmfltr");
    ...
    // /* kernel fuzzing */
    // DeviceIoControl(kafl_vuln_handle1,
    //     0x2224C8,
